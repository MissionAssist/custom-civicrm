...
{foreach from=$rows item=row}
  <tr id='rowid{$row.participant_id}' class="{cycle values="odd-row,even-row"} crm-event crm-event_{$row.event_id}">
    ...
    {* Start MissionAssist
     MA wants to see payment amount and the balance*}
    <td class="right nowrap crm-participant-participant_fee_amount">{if array_key_exists('paid', $row)}{$row.paid|crmMoney:$row.participant_fee_currency}{/if}</td>    
    <td class="right nowrap crm-participant-participant_fee_amount">{if array_key_exists('balance', $row)}{$row.balance|crmMoney:$row.participant_fee_currency}{/if}</td>
    {* End MissionAssist *}
