...
function joomla_civicrm_tokens(&$tokens) {
  ...
  $tokens['Skill'] = array(
    'Skill.attributes_HTML' => 'List of skills in HTML format',
  ...
  $tokens['Attribute'] = array(
    'Attribute.attributes_HTML' => 'List of Other Attributes in HTML format',
    'Attribute.attributes_Text' => 'List of Other Attributes in text format',
  );
}
/*
  ...
  foreach ($passed_tokens as $group => $token)
  {
    ...
    if (is_array($tokens['Email'])) {
      ...
      foreach($tokens['Email'] as $token => $value)
      {
        ...
        if (is_array($tokens['Attribute']))
    {
        /*
         * If we have tokens associated with attributes, so loop through them.
         */
        foreach($tokens['Attribute'] as $token => $value)
        {
            switch($token)
            {
                case 'attributes_HTML':
                    /*
                     * Get the attributes for contacts being emailed in HTML format.
                     */
                    $attributestable = get_values_for_attributes_tokens($cids);
                    foreach($attributestable as $contactID => $attributes)
                    {
                        if (count($attributes) > 1)
                        {
                             ...
                             * There will always be a header.
                             */
                            $attributes_list = '<table cellspacing = "10"><tbody>';
                            foreach($attributes as $skillid => $data)
                            {
                                if ($skillid === 'header')
                                {
                                    $fieldstart = '<th>';
                                ...
                                foreach($data as $field)
                                {
                                    $attributes_list .= $fieldstart . $field . $fieldend;
                                }
                                $attributes_list .= '</tr>';
                            }
                            $attributes_list .= '</tbody></table>';
                            $values[$contactID]['Attribute.attributes_HTML'] = $attributes_list;

                        }
                        else
                        {
                                $values[$contactID]['Attributes.attributes_HTML'] = "You don't seem to have any attributes noted by MissionAssist";
                        }

                    }
                    break;
                case 'attributes_Text':
                    $attributestable = get_values_for_attributes_tokens($cids);
                    ...
                    foreach($attributestable as $contactID => $attributes)
                    {
                        if (count($attributes) > 1)
                        {
                             ...
                             */
                            $attributes_list = '';
                            foreach($attributes as $cid => $data)
                            {

                                foreach($data as $field)
                                {
                                    $attributes_list .= str_pad($field, 25);
                                }
                                $attributes_list .= "\n";

                            }
                            $values[$contactID]['Attribute.attributes_Text'] = $attributes_list;
                        }
                        else
                        {
                            $values[$contactID]['Attribute.attributes_Text'] = "You don't seem to have any attributes noted by MissionAssist";
                        }
...
function get_values_for_attributes_tokens($cids)
{
    /*
     * Get the list of attributes
     */
    /*
     * First look up the custom fields associated with Other Attributes
     * 
     */
    require_once 'CRM/Core/BAO/CustomField.php';

    $skillFieldID = CRM_Core_BAO_CustomField::getCustomFieldID( 'Attribute', 'Other Attributes' );
    $noteFieldID = CRM_Core_BAO_CustomField::getCustomFieldID( 'Note', 'Other Attributes' );

    $custom_fields = array('Attribute' => "custom_" . $skillFieldID,
        'Note' => "custom_" . $noteFieldID );

    $attribut_options  = civicrm_api3('Contact', 'getoptions', array(
        'sequential' => 1,
        'field' => $custom_fields['Other Attributes'],
      ));
    foreach($cids as $contactID)
    {
        /*
         * Now get the values for each contact.
         */
        $attributeslist[$contactID] = array();
        $attributeslist[$contactID]['header'] = array('Skill', 'Note');

        $params = array(
            'entityID' => $contactID,
            $custom_fields['Attribute'] => 1,
            $custom_fields['Note'] => 1,
           );
        $attributes_table = array();  // set up a skeels table
        $attributes_values = CRM_Core_BAO_CustomValueTable::getValues($params);
        // We need to sort the fields into two columns based on their field ids and rows by idividual id
        if (!$attributes_values['is_error'])
        {
            foreach($attributes_values as $key => $value)
            {
               ...
               {
                    $ids = preg_split('/_/', $key);
                    $attributes_table[$ids[2]][$ids[1]] = $value;
               }
            }
        }


        foreach($attributes_table as $id => $fields)
        {

            $attributeslist[$contactID][$id] = $fields;
