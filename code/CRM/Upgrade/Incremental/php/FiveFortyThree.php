...
class CRM_Upgrade_Incremental_php_FiveFortyThree extends CRM_Upgrade_Incremental_Base {
  ...
  public static function setNoteDateFieldNotNull(): bool {
    $i18nRewrite = FALSE;
    // We want to make sure the columns exist before trying to change them.
    CRM_Upgrade_Incremental_php_FiveFortyThree::makeColumn("civicrm_note", "note_date", "TIMESTAMP");
    CRM_Upgrade_Incremental_php_FiveFortyThree::makeColumn("civicrm_note", "created_date", "TIMESTAMP");
    ...
    $query = "ALTER TABLE civicrm_note CHANGE COLUMN `note_date` `note_date` timestamp NULL DEFAULT current_timestamp() COMMENT 'Date attached to the note'";
    CRM_Core_DAO::executeQuery($query, [], TRUE, NULL, FALSE, $i18nRewrite);
    return TRUE;
  }
  public static function makeColumn(string $theTable, string $theColumn, string $theDateType) {
       /* 
       * Check if the column name exists and if it doesn't, create it
       */
      $i18nRewrite = FALSE;
      $query = "SELECT * FROM information_schema.COLUMNS
        WHERE COLUMN_NAME='".$theColumn."' AND TABLE_SCHEMA = 'civicrm' AND TABLE_NAME='".$theTable."'";
      $count = CRM_Core_DAO::executeQuery($query, [], TRUE, NULL, FALSE, $i18nRewrite);
      if (!$count->N) {
        $query = "ALTER TABLE `civicrm`.`".$theTable."` ADD COLUMN `".$theColumn."` ".$theDateType." NULL";
        CRM_Core_DAO::executeQuery($query, [], TRUE, NULL, FALSE, $i18nRewrite);
      }
  }
}
