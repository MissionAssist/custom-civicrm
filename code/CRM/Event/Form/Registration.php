...
class CRM_Event_Form_Registration extends CRM_Core_Form {
  ...
  public static function initEventFee(&$form, $eventID, $includeExpiredFields = TRUE) {
    ...
    $discountId = CRM_Core_BAO_Discount::findSet($eventID, 'civicrm_event');
      /*
       * Retrieve line items for all participants registered by this partipant
       */
      $participants = [];
      $lineItems = [];
      $oldID = $form->_id; // Remember the ID.
      $participants += [$form->_id];
      $participants += CRM_Event_BAO_Participant::getAdditionalParticipantIDs($oldID);
      foreach ($participants as $ID) {
        ...
        $lineItems += $form->_values['line_items'];
      }
      $form->_id = $oldID; // Make sure are using the proper ID
      $form->_values['line_items'] = $lineItems;
