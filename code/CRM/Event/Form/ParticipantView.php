...
class CRM_Event_Form_ParticipantView extends CRM_Core_Form {
  ...
  public function preProcess() {
    ...
    if (CRM_Event_BAO_Participant::isPrimaryParticipant($participantID)) {
      $participants += CRM_Event_BAO_Participant::getAdditionalParticipantIDs($participantID, false, null);
      $values[$participantID]['additionalParticipants'] = CRM_Event_BAO_Participant::getAdditionalParticipants($participantID);
      foreach (CRM_Event_BAO_Participant::getAdditionalParticipantIDs($participantID, false, null) as $additionalID) {
          // Get the information about the additional participants.
    ...
    if (CRM_Event_BAO_Participant::isPrimaryParticipant($participantID)) {
      // Get Line Items for each participant registered.

      $lineItem = array();
      foreach ($participants as $ID) {
        $lineItem += CRM_Price_BAO_LineItem::getLineItems($ID);
      }
      ...
      // Calculate the total amount for all participants.
      $totalAmount=0.0;
      foreach ($participants as $ID) {
        $totalAmount += CRM_Utils_Array::value('fee_amount', $values[$ID]);
      }
      $values[$participantID]['totalAmount'] = $totalAmount;
    }
