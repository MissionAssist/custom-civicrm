...
class CRM_Event_Selector_Search extends CRM_Core_Selector_Base implements CRM_Core_Selector_API {
  ...
  public function getPaymentStatus($row)
  {
      ...
      {
        /*
         * Only reveal payment data it the user has the right to edit partipant information.
         * This is so someone who just wants to see who is coming doesn't see home much they have paid.
         */
        $participant_id = $row['participant_id'];  // Remember the participant ID as we are using it a lot.
        $params = ['registered_by_id'];
        $registered_by_id = CRM_Event_BAO_Participant::getRegisteredByID($participant_id, $params);
        if ($registered_by_id == NULL)
           {
            /*
             * The participant was not registered by someone else so payment i
             */

            $row['is_primary'] = TRUE;
            $nopaymentdata = ['paid'=>'0.00', 'payLater'=>false];  // default values
            $nopaymentdata['balance'] = $row['participant_fee_amount'];
            $paymentInfo= CRM_Contribute_BAO_Contribution::getPaymentInfo($participant_id, 'event', FALSE, TRUE);
            if (is_array($paymentInfo))
              {
                 foreach($paymentInfo as &$amount)
                 {
                     if (is_numeric($amount))
                     {
                        $amount = number_format($amount, 2); // two decimal places
                     }
                 }
                 $row = array_merge($row, $paymentInfo);
                 $row['participant_fee_amount'] = $paymentInfo['total'];  // getting the payment info also gets the total amount owed.
              }
            else
              {
                $row = array_merge($row, $nopaymentdata);
                /*
                 * Now get the fee amounts for participants registered by this participant and add them up
                 * He or she is repsonsible for paying for the others.
                 */
                $additionalparticipants = CRM_Event_BAO_Participant::getAdditionalParticipantIds($participant_id);
                ...
                {
                    $row['participant_fee_amount'] = $row['participant_fee_amount'] + floatval($fee['fee_amount']);
                }

              }
              $row['participant_fee_amount'] = number_format($row['participant_fee_amount'], 2); // give it two numbers after the decimal point.
        } else {
            /*
             * The participant was registered by someone else, so we refer to them
             */
